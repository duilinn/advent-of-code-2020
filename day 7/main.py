rules = """light green bags contain 2 pale cyan bags.
...
rules = rules.replace(" no other bags.","").split("\n")
...
for i in range(len(rules)):
    ...
    #print(split)
    newRule = []
    newRule.append([split[0] + " " + split[1], "1"])

    for j in range(4,len(split), 4):
        
        newRule.append([split[j+1] + " " + split[j+2], split[j]])
...
def search(bagColour):
    for rule in rules:
        for colour in rule[1:]:
            if bagColour == colour[0]:

                if str(rule[0][0]) not in outerColours:
                    outerColours.append(rule[0][0])
                    search(str(rule[0][0]))
search("shiny gold")
...
def searchCount(bagColour, depth, bagMultiplier):
    #print("#####")
    colourFound = False
    for rule in rules:
        
        if rule[0][0] == bagColour:
            colourFound = True
            for colour in rule[1:]:
                for x in range(depth):
                    print("\t",end="")
                print(str(colour) + ", m =" + str(bagMultiplier))
                innerBagsCount.append(bagMultiplier*int(colour[1]))
                if True:
                    innerColours.append(colour[0])
                    searchCount(colour[0], depth+1, bagMultiplier*int(colour[1]))
#part 1
print(len(outerColours))
#part 2
innerBagsCount = []
searchCount("shiny gold",0,1)
finalInnerBagsCount = 0

for innerBag in innerBagsCount:
    finalInnerBagsCount += innerBag

print(finalInnerBagsCount)
