...
for i in range(len(text)):
    for j in range(len(text[i])):
        #print("i = " + str(i) + ", j = " + str(j) + ": " + text[i][:j] + "[" + text[i][j] + "]" + text[i][j+1:])
        
        if (text[i][j] not in text[i][:j]) or (j == 0):
            totalLen += 1
            #print("match, totalLen = " + str(totalLen))
        else:
            pass
            #print("no match, totalLen = " + str(totalLen))
        #print("~~~")
