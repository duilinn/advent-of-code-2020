data = """LLLLL.LLLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLL.LLLLLLLLLL.LLLLLLLLLLL.LLLLLLLLLLLLLL.LLLL.LLLL.LLLLLL
...
def seatScan(grid,x,y,xStep,yStep):
    currentSeat = "."
    fullSeatFound = False
    while (currentSeat=="." and x>=0 and y>=0 and y<len(grid) and x<len(grid[y])):
        #print("currentSeat =" + currentSeat + ", xStep = " + str(xStep) + ", yStep = " + str(yStep))
        x += xStep
        y += yStep
...
def neighbourCount(grid,x,y):
    count = 0
    #print("test, x = " + str(x) + ", " + str(y))
    if (x>0 and y>0):
        count += seatScan(grid,x,y,-1,-1)
    if (y>0):
        count += seatScan(grid,x,y,0,-1)
    if (x<len(grid[y])-1 and y>0):
        count += seatScan(grid,x,y,1,-1)
        
    if (x>0):
        count += seatScan(grid,x,y,-1,0)
    if (x<len(grid[y])-1):
        count += seatScan(grid,x,y,1,0)
        #print(seatScan(grid,x,y,1,0))
...
while(not finished):
    ...
    for j in range(len(data)):
        ...
        for i in range(len(data[j])):
            if data[j][i]=="L":
                #print(neighbourCount(data,i,j), end = "")
                if neighbourCount(data,i,j)==0:
                    newRow += "#"
                else:
                    newRow += "L"
